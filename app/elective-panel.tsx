...
import {catalogCategory,catalogCategoryLabels,matchesCourseQuery,type CatalogCategory,type ElectiveOption} from './elective-logic';
...
export default function ElectivePanel({options,selected,previewed,year,major,issuesFor,onToggle,onPreview,onDetails,onClear}:{
  ...
  const rows=options.map(option=>({option,issues:issuesFor(option)})).filter(({option,issues})=>{
    ...
    return matchesCourseQuery(option,query);
  });
  return <>
    ...
    <label className="courseSearch"><span>{year===5?'搜索课程':'搜索选修课'}</span><input type="search" value={query} onChange={event=>setQuery(event.target.value)} placeholder="课程名、课程类别、教师或教学班"/></label>
    {year===5&&<div className="catalogFilters">
      ...
      <div className="filterPills catalogCategoryTabs" role="group" aria-label="按课程类别筛选">{(['all','jnu','english','general','single'] as const).map(value=><button type="button" key={value} aria-pressed={category===value} className={category===value?'active':''} onClick={()=>setCategory(value)}>{value==='all'?'全部类别':value==='general'?'通识课':catalogCategoryLabels[value]}</button>)}</div>
    </div>}
    ...
    <div className="sidebarCourseList electiveCourseList">{rows.map(({option,issues})=>{
      ...
      return <article className={`retakeCourseCard ${year===2?'category-english':'category-jnu'} ${chosen?'selected':''} ${severity==='hard'?'hasHardConflict':severity==='partial'?'hasPartialConflict':''}`} key={option.key}>
        ...
        {option.events.length>1&&<small>共 {option.events.length} 个上课时间段 · 一并加入或移除</small>}
        {option.events.map((session,index)=><p className="electiveSession" key={session.id}>{option.events.length>1&&<><b>时间段 {index+1} · {session.title.includes('实验')?'实验':'课程'}</b><br/></>}{session.listedOnly?'时间待通知':`${days[session.day]} ${sessionTimeLabel(session,times)}`}<br/>教室：{roomForMajor(session,major)}<br/>周次：{session.weeks||'待通知'}{session.teacher&&<><br/>教师：{session.teacher}</>}</p>)}
