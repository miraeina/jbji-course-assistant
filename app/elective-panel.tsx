...
export default function ElectivePanel({options,selected,year,issuesFor,onToggle,onPreview,onDetails}:{
  ...
  return <>
    <div className="retakeIntro"><strong>{year===2?'选择英语课程与教学班':'选择本学期选修课'} · 已选 {selected.length} 门</strong><p>用于个人排课，正式选课以学校系统为准。展示原课表适用于当前专业、班级的课程。</p>{year===2&&<p>原文件注明：2025级需选 1 门英语模块。雅思按原教学班展示；同一课程切换班次会替换原选择。</p>}</div>
    ...
    <div className="sidebarCourseList electiveCourseList">{rows.map(({option,issues})=>{
      ...
      return <article className={`retakeCourseCard ${year===2?'category-english':'category-jnu'} ${chosen?'selected':''} ${severity==='hard'?'hasHardConflict':severity==='partial'?'hasPartialConflict':''}`} key={option.key}>
        ...
        {option.events.map(session=><p className="electiveSession" key={session.id}>{session.listedOnly?'时间待通知':`${days[session.day]} ${times[session.start]?.[1]}–${times[session.start+session.span-1]?.[2]}`}<br/>{session.room||'教室待通知'} · {session.weeks||'周次待通知'}{session.teacher&&<><br/>{session.teacher}</>}</p>)}
